...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        ...
        if i == 0:  # top left
            ...
            img4[0:yc, 0:xc,:] = transformed["image"]
            if len(transformed["bboxes"]):
                labels = np.array(transformed["bboxes"])
        elif i == 1:  # top right
            crop_edge = max(int(1.43 * yc), int(1.43 * (imsize - xc)))
            RANDOM_CROP_ROTATED_CENTRER_CROP = A.Compose([  
                                                            A.RandomCrop(height=crop_edge, width=crop_edge, p=1.0), # sqrt(2) < 1.42
                                                            A.Rotate(limit=170, border_mode=1, p=1.0),
                                                            A.CenterCrop(height=yc, width=imsize - xc, p=1.0),  
                                                            ], bbox_params=A.BboxParams(format="pascal_voc", label_fields=["bbox_classes"]))
            transformed = RANDOM_CROP_ROTATED_CENTRER_CROP(image=img, bboxes=labels, bbox_classes=cats)

            img4[0:yc, xc:imsize, :] = transformed["image"]
            if len(transformed["bboxes"]):
                labels = np.array(transformed["bboxes"])
                labels[:,[0,2]] += xc
        elif i == 2:  # bottom left
            crop_edge = max(int(1.43 * (imsize - yc)), int(1.43 * xc))
            RANDOM_CROP_ROTATED_CENTRER_CROP = A.Compose([  
                                                            A.RandomCrop(height=crop_edge, width=crop_edge, p=1.0), # sqrt(2) < 1.42
                                                            A.Rotate(limit=170, border_mode=1, p=1.0),
                                                            A.CenterCrop(height=imsize - yc, width=xc, p=1.0),  
                                                            ], bbox_params=A.BboxParams(format="pascal_voc", label_fields=["bbox_classes"]))
            transformed = RANDOM_CROP_ROTATED_CENTRER_CROP(image=img, bboxes=labels, bbox_classes=cats)

            img4[yc:imsize, 0:xc, :] = transformed["image"]
            if len(transformed["bboxes"]):
                labels = np.array(transformed["bboxes"])
                labels[:,[1,3]] += yc
        elif i == 3:  # bottom right
            crop_edge = max(int(1.43 * (imsize - yc)), int(1.43 * (imsize - xc)))
            RANDOM_CROP_ROTATED_CENTRER_CROP = A.Compose([  
                                                            A.RandomCrop(height=crop_edge, width=crop_edge, p=1.0), # sqrt(2) < 1.42
                                                            A.Rotate(limit=170, border_mode=1, p=1.0),
                                                            A.CenterCrop(height=imsize - yc, width=imsize - xc, p=1.0),  
                                                            A.HorizontalFlip(p=0.65),
                                                            A.VerticalFlip(p=0.65),
                                                            ], bbox_params=A.BboxParams(format="pascal_voc", label_fields=["bbox_classes"]))
            transformed = RANDOM_CROP_ROTATED_CENTRER_CROP(image=img, bboxes=labels, bbox_classes=cats)
            
            img4[yc:imsize, xc:imsize, :] = transformed["image"]
            if len(transformed["bboxes"]):
                labels = np.array(transformed["bboxes"])
                labels[:,[0,2]] += xc
                labels[:,[1,3]] += yc
        ...
        if len(transformed["bboxes"]):
            ...
            for idx, label in enumerate(labels):
                if (label[2]- label[0]) < min_edge or (label[3]- label[1]) < min_edge: 
                    np.delete(labels, idx, 0)
            # Labels
            labels = np.column_stack((np.transpose(np.zeros(labels.shape[0], dtype=int)), labels))
    ...
    if len(labels4):
        ...
        for x in labels4[:, 1:]:
            np.clip(x, 0, imsize, out=x)  # clip when using random_perspective()
    else:
        labels4 = np.array([])
    ...
    # im.save("ducanh.jpg", 'JPEG')         # save image
    return img4, labels4
